...
#ifndef TPOOL_THREAD_NAME
#define TPOOL_THREAD_NAME tpool
#endif
...
typedef struct thread
{
    ...
    struct tpool_ *tpool_p; /* access to tpool          */
} thread;
...
static int thread_init(tpool_ *tpool_p, struct thread **thread_p, int id);
...
struct tpool_ *tpool_init(int num_threads)
{
    ...
    tpool_ *tpool_p;
    tpool_p = (struct tpool_ *)malloc(sizeof(struct tpool_));
    if (tpool_p == NULL)
    {
...
int tpool_add_work(tpool_ *tpool_p, void (*function_p)(void *), void *arg_p)
{
    job *newjob;
...
void tpool_wait(tpool_ *tpool_p)
{
...
void tpool_destroy(tpool_ *tpool_p)
{
...
void tpool_pause(tpool_ *tpool_p)
{
    int n;
    for (n = 0; n < tpool_p->num_threads_alive; n++)
    {
        pthread_kill(tpool_p->threads[n]->pthread, SIGUSR1);
    }
}

/* Resume all threads in threadpool */
void tpool_resume(tpool_ *tpool_p)
{
...
int tpool_num_threads_working(tpool_ *tpool_p)
{
...
static void *thread_do(struct thread *thread_p)
{
    ...
    snprintf(thread_name, 16, TOSTRING(TPOOL_THREAD_NAME) "-%d", thread_p->id);
    ...
    tpool_ *tpool_p = thread_p->tpool_p;
